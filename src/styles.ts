...
export const styles = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* exhibits read as their own kind in listings, not as generic markdown */
    .item-kind-exhibit { color: var(--accent); border-color: var(--accent); }
...
.graph-archived-toggle { display: inline-block; margin-left: 0.75rem; font-size: 0.7rem; color: var(--text-dim); text-transform: uppercase; letter-spacing: 0.06em; }
    ...
    /* --- Exhibit view (view: exhibit markdown items) --- */
    /* Exhibit pages widen the WHOLE container (header, breadcrumb, exhibit) rather
    ...
    /* Kiosk (?kiosk=1): the exhibit alone, edge to edge — the recordable surface. */
    ...
    .container-kiosk #exhibit-wrap { margin-top: 0; border: none; }
    #exhibit-wrap { position: relative; margin-top: 0.8rem; border: 1px solid var(--border); background: var(--bg); overflow: hidden; }
    #exhibit-canvas { display: block; width: 100%; cursor: grab; touch-action: none; }
    #exhibit-canvas:active { cursor: grabbing; }
    #exhibit-empty { padding: 3rem; text-align: center; }
